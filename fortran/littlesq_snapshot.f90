...
! simulations for a given combination of model parameters, and returns
! a snapshot of the lattice at the end of the simulation (whitespace-
! separated table in outfile).
!
! The program takes four command-line arguments: the first argument is
! the desired tau, the second is p_i, the third is the filename for the
! outfile, and the fourth is p_e. Other parameters need to be set using
! preprocessor directives, see directly below.
...
! iterations, which will be N*N*M.
#define _N_ 50
...
! The program itself
program littlesq_snapshot
  ! Declare variables
  implicit none
  character(len=32) :: comarg1, comarg2, comarg3, comarg4 ! command-line arguments
  integer :: N, iter, nact, ups ! stuff
