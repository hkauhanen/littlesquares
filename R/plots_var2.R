...
plot_everything <- function(plot_interpolations = TRUE) {
  # interpolations
  ...
  pdf("../plots/simutheory.pdf", height=4, width=4)
  ggplot.simu_and_theory()
  dev.off()

  # main result
  pdf("../plots/mainresult.pdf", height=4, width=4)
  ggplot.main_result()
  dev.off()
...
ggplot.main_result <- function(df = read.csv("../data/main-analysis.csv")) {
  ...
  g <- ggplot(df, aes(x=feature_frequency, y=isogloss_density, group=feature_ID)) 
  ...
  g <- g + geom_point(data=dfmed, aes(x=feature_frequency, y=isogloss_density)) + geom_text_repel(data=dfmed, aes(label=feature_ID), box.padding=0.75, alpha=0.8)
...
# Main result; features on a sigma-against-rho plot, with confidence ellipses
ggplot.main_result.original <- function(df = read.csv("../data/main-analysis.csv")) {
  commonalpha <- 1
  dfmed <- aggregate(df[, 2:4], list(df$feature_ID), median, na.rm=TRUE)
  names(dfmed)[1] <- "feature_ID"
  ...
  g <- g + stat_ellipse(alpha=0.3)
  #g <- g + geom_point(data=dfmed, aes(x=feature_frequency, y=isogloss_density, color=feature_ID)) + geom_text_repel(data=dfmed, aes(label=feature_ID), box.padding=0.75, alpha=commonalpha)
  g <- g + geom_point(data=dfmed, aes(x=feature_frequency, y=isogloss_density)) + geom_text_repel(data=dfmed, aes(label=feature_ID), box.padding=0.75, alpha=0.7)
  g <- g + theme_bw() + xlim(0, 1) + ylim(0, 0.5) + theme(legend.position="none")
  mycolors <- colorRampPalette(brewer.pal(8, "Spectral"))(nrow(dfmed))
  #g <- g + scale_color_manual(values=brightness(saturation(mycolors, 1), 0.9))
  #g <- g + scale_color_viridis()
...
# Main result; features on a sigma-against-rho plot, with confidence ellipses
ggplot.main_result.dark <- function(df = read.csv("../data/main-analysis.csv")) {
  commonalpha <- 1
  dfmed <- aggregate(df[, 2:4], list(df$feature_ID), median, na.rm=TRUE)
  names(dfmed)[1] <- "feature_ID"
  ...
  #g <- ggplot(df, aes(x=feature_frequency, y=isogloss_density, color=feature_ID)) + stat_ellipse(alpha=commonalpha)
  g <- ggplot(df, aes(x=feature_frequency, y=isogloss_density, group=feature_ID)) 
  g <- g + geom_line(data=parabs, aes(x=feature_frequency, y=isogloss_density, color=log(temperature)), alpha=0.6)
  g <- g + stat_ellipse(alpha=0.4, color="white")
  #g <- g + geom_point(data=dfmed, aes(x=feature_frequency, y=isogloss_density, color=feature_ID)) + geom_text_repel(data=dfmed, aes(label=feature_ID), box.padding=0.75, alpha=commonalpha)
  g <- g + geom_point(data=dfmed, aes(x=feature_frequency, y=isogloss_density), color="white") + geom_label_repel(data=dfmed, aes(label=feature_ID), box.padding=1.75, alpha=0.7, color="white", bg="black")
  g <- g + theme_bw() + xlim(0, 1) + ylim(0, 0.5) + theme(legend.position="none", panel.background=element_rect(fill="#111111"), panel.grid.major=element_line(color="#222222"))
  mycolors <- colorRampPalette(brewer.pal(8, "Spectral"))(nrow(dfmed))
  #g <- g + scale_color_manual(values=brightness(saturation(mycolors, 1), 0.9))
  #g <- g + scale_color_viridis()
  g <- g + scale_color_distiller(palette="RdYlBu")
